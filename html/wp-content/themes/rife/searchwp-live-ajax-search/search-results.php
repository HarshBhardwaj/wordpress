...
<!--		--><?php //$post_type = get_post_type_object( get_post_type() ); echo esc_html( $post_type->labels->singular_name ); ?>
	...
	?>
<div><a href="<?php echo esc_url( apollo13framework_current_url().'?s='.$_search ); ?>" class="all-results"><?php esc_html_e('View all results', 'rife'); ?></a></div>
<?php else : ?>
	<div class="searchwp-live-search-no-results">
		<h2><?php esc_html_e( 'No results found.', 'rife' ); ?></h2>
		<em><?php esc_html_e( 'Please try another search term', 'rife' ); ?></em>
	</div>
<?php endif; ?>
